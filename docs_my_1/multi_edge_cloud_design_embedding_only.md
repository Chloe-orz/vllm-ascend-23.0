...
> 版本：v1.6（2026-08-14；…；v1.5 = §2.3.1 静态通信域多边初始化清单与限制；v1.6 = 新增 §2.3.2 全局域 vs 全相联 P2P 对比设计）
...
Edge 收到请求
  ...
  2. Route RPC → 中央: {edge_id, block_hashes, prompt_len, priority}
        ...
        —— lease_id = 路由时中央请云 pin 住命中 prefix 块的"预订凭证"句柄，
           PF 到达时云 confirm_lease 把 pin 引用转给请求；超时未到达自动解 pin（§4.3c）
...
> **注 1（req_id 全局唯一：云侧包装，边不感知）**：vllm 默认 `random_uuid()`，但客户端可显式
> 指定 request_id（`chat_completion/protocol.py:352`），两边用户传相同 id 会撞。**前缀化不做在
> 边侧，而做在云侧入口适配层**：云对每个边有独立 channel，收包时 channel 即标识 edge_id，
> 云在入口统一包装 `cloud_req_id = f"e{edge_id}-{orig_req_id}"`（head_token 同理包成
> `f"{edge_id}:{orig_token}"`），云内所有结构（registry、pending_heads、prepare cache 键、
> 调度队列、日志）一律使用包装后的 id；**云→边的所有回传（c2e 数据面、NACK、Pause/Resume、
> PreemptNotice 等控制消息）在出口适配层统一剥离前缀**，边看到的永远是自己的原始 id。
> 效果：边侧零改动、完全不感知多边命名空间；云内键协议保证全局唯一；消息线上不传 edge_id。
> 纪律：所有云→边消息必须经出口适配器剥离，漏一处 = 边不认识自己的请求（纳入 code review
> 检查点与单测）。
...
| **KV 配置** | **每云独立**决定自己的 `num_blocks`/spec（不再全集群统一 clamp；边的虚拟显存 hack 删除） |
...
每步的文字化说明见 §2.6，此处仅保留骨架：

```
 客户端      边 E0（embed+采样，无KV）   中央调度器        云 C1（全模型层+KV池）     云 C2
   │              │                     │                  │                     │
   │ 请求         │                     │                  │                     │
   │─────────────▶│                     │                  │                     │
   │              │ 1. tokenize + block_hashes             │                     │
   │              │ 2. Route RPC ──────▶ │                  │                     │
   │              │                     │ GPI 前缀匹配 + 负载打分 + 一致性哈希      │
   │              │                     │ 3. pin{lease} ──▶│                     │
   │              │ 4. ◀── {C1, est_hit, lease_id}         │                     │
   │              │    记路由表 req→(C1, lease)            │                     │
   │              │ 5. embed 全量        │                  │                     │
   │              │ 6. PF 段 ══ ZMQ 控制面 ══════════════▶│                     │
   │              │ 7. embeddings ═══ HCCL 数据面 ════════▶│                     │
   │              │                     │                  │ 8. 入口包装 id        │
   │              │                     │                  │ 9. 准入 ─拒绝─▶ NACK  │
   │              │◀════════════════ NACK（可选支线）══════╡                     │
   │              │ 重新 Route ────────▶ │ 改路由 C2 ───────────────────────────▶│
   │              │                     │                  │ 10. confirm_lease    │
   │              │                     │  ◀─ pending/solid│ 11. 算哈希+allocate  │
   │              │                     │    + insert      │    （权威命中）        │
   │              │                     │    （GPI 上报）   │ 12. 注册 → 入队       │
   │              │                     │                  │ 13. 出队执行          │
   │              │                     │                  │    （CHER/切层/穿插）  │
   │              │ 14. ◀══ c2e 末位置 hidden+residual ═══│                     │
   │ 首 token     │ 15. norm+lm_head+采样│                  │                     │
   │◀─────────────│                     │                  │                     │
   │              │ 16. decode 循环：DF → 增长分配 → 执行 → c2e → 采样              │
   │◀─────────────│    （复用路由表，不再 Route；MTP 同通道交替）│                  │
   │              │ 17. FIN ────────────┼─────────────────▶│ 18. free → 块转空闲  │
   │              │                     │                  │     prefix（跨边共享）│
```
...
```
过载: L1 空闲驱逐 → L2 Pause+D2H 卸载+Resume → L3 PreemptNotice 重算（防雪崩三件套）
异常: 边死=SUSPECT/DEAD+TTL GC（边零损失）｜云死=广播+重路由全量重算｜FIN 竞态=配对接完再清理
```
...
请求到达边 E0 后，边做 tokenize 并按 vllm 原生链式哈希算出整段 prompt 的 block_hashes——这是
纯 CPU 操作，不需要跑模型。随后边向中央调度器发起一次 Route RPC，携带 block_hashes、prompt
长度和优先级。中央用 GPI 对每个候选云做最长前缀匹配（估算该云能省多少算力），结合各云心跳上报
的 KV 余量、队列深度、近期抢占记录打分，平局时按首个 block hash 做一致性哈希（让同前缀天然聚集
同云、命中率随时间上升）。决策完成后中央做两件事：把所选云的前缀 hashes 在 GPI 里标为 PENDING
（让紧随的同前缀请求蹭在途 prefill），并向所选云发 pin 租约请求（云可拒绝或部分接受——pin 只是
优化，不是资源分配决定）。最后中央把 {cloud_id, est_hit, lease_id} 返回给边。边在本地路由表记下
req_id → (cloud_id, lease_id)，**这就是这个请求整个生命周期里唯一一次路由决策**。

**阶段二：prefill 下发与云侧准入**

边把整段 prompt 做 embedding（查表，零上下文依赖），**全量**发往 C1——不按 est_hit 截断，因为云
自管 KV 需要全量 token ids 在本地重建链式哈希，且全量发送保证"云实命中低于预期"时输入依然齐备。
控制面 PF 段（全量 token ids + lease_id + 隐藏通道号）经 pair 的 ZMQ 通道先行，数据面 embeddings
经 pair 的 HCCL group 异步 isend。云的收包入口做两件多边适配：一是收包 channel 即标识来源边，
**入口适配层把 req_id/head_token 包装成云内全局唯一键**（`e{edge_id}-` 前缀，边完全不感知，
回传时再剥离）；二是准入检查——逐边配额、全局 inflight 上限、KV 余量（扣除 pinned 与在队段持有），
不满足且可拒绝则沿收包 channel 直接回 NACK（边收到后重新 Route，中央会临时调低该云权重）。
准入通过则 confirm_lease（租约引用转给请求）、用全量 token ids 算哈希、allocate_slots 做**权威
命中**（云本地块池才是 truth），登记请求注册表，段入全局队尾。

**阶段三：云侧调度执行与首 token 回传**

云调度器只有一张全局队列，按（优先级档， 到达序号）出队，唯一约束是"同边不可超越"——同一
边的段严格按到达序执行。出队时才从段内包装后的 req_id 解析 edge_id，选该 pair 的 HCCL group。
执行前 CHER guard 线程提前 irecv、`cloud_prepare_early` 提前构建 attention metadata，与边的
segment 计算重叠。大 prefill 可按 token 阈值切层，slice 之间可插入任一边的 decode/draft（全云
同时只允许一个 sliced prefill 续算）。段完成时 `cache_blocks` 把新算的块插入 hash 映射并攒批
上报 GPI。c2e 只需回传**末位置的 hidden+residual**（em 边尾是 position-wise 的 norm+lm_head），
边做 final norm + lm_head + 采样，产出首 token。

**阶段四：decode 循环（复用路由，不再问中央）**

边每步从本地路由表查到 C1，直发 DF 段（每请求 1 个 token 的 embedding）。云收到后查请求注册表、
做增长分配（每 token 一个槽位），执行中间层，c2e 回传，边采样出 token。MTP 场景下 DF/DL 与
DRF/DRL 严格交替，draft 步共用 DECODE 通道，他边的段可以穿插其间（只延迟不错序，共享 buffer
由快照/恢复保护）。这个循环直到 EOS 或达到长度上限。

**阶段五：结束与释放**

请求完成或客户端 abort 时，边发**显式 FIN 控制消息**（独立通道、重传+幂等，不搭 SO 便车——多边
下某边长期无 SO 到某云是常态）。云收到 FIN：若该请求无在飞段，直接 free——块转空闲 prefix 缓存
（hash 映射保留，**可被其他任何边的同前缀请求命中**，这就是跨边共享）；若有在飞段，标记 ZOMBIE，
段收尾后结果丢弃不回传（通道 FIFO 不可掐断），再释放。边侧删本地路由记录。

**支线一：云过载**——decode 增长分配失败时按四级处置：先靠准入/NACK 不让它满；其次空闲 prefix
驱逐（原生 LRU，零代价）；再主动卸载（PauseReq 让边停发 → D2H 到 host DDR → ResumeReq 恢复）；
兜底协同重算（PreemptNotice → 边重发全量 PF，em 下重 embed 零成本）。水位 0.95/0.80 滞后 +
每分钟抢占上限 + 中央降权防雪崩。

**支线二：异常**——边死亡：心跳超时进入 SUSPECT/DEAD，云剔除其段、orphan 块转空闲、TTL GC 兜底，
em 下边重启零状态损失（无 KV）；云死亡：中央广播，边扫本地路由表把指向死云的请求重路由，新云
全量重算（仅需边重发 embeddings）；丢包：head_token 幂等去重重发；重启身份混淆：(id, epoch)
三元组，新 epoch 旧状态全作废。
...
### 4.2.1 边→云 KV 信息传递变迁详表

**原来（边管 KV）边向云传递的 KV 相关信息**（字段见 `vllm/vllm/v1/core/sched/output.py`）：

| 时机 | 信息 | 说明 |
|---|---|---|
| 初始化（一次性） | **KVCacheConfig**（`num_blocks`、`block_size`、kv_cache_groups/spec） | `get_kv_cache_configs` 全 worker 合并统一、`num_blocks` clamp 到最小（`kv_cache_utils.py:2061-2207`）；em 边报 1TiB 虚拟显存让云定块数（`worker.py:755-772`）——块总量与规格是边云协商出来的 |
| 新请求首调（`NewRequestData`） | **`block_ids`** | 完整块表（按 group 分组）——边 `allocate_slots` 的结果，云 worker 照此寻址写 KV |
| 同上 | **`num_computed_tokens`** | **边 prefix 命中判定的结果**，云照抄，决定从哪个位置开始算 |
| 在途每步（`CachedRequestData`） | **`new_block_ids`** | 增量块表（decode 增长/chunk 推进）；对 `resumed_req_ids` 中的请求是整块表替换语义 |
| 同上 | **`num_computed_tokens`** / `resumed_req_ids` | 最新进度 / 抢占恢复标记 |
| 全局 | **`finished_req_ids`** | 释放通知，搭车下一个非空 SO（`patch_engine_core.py:351-368` 合并 deferred EMPTY） |
| 全局 | **`new_block_ids_to_zero`** 等 | 块池内部维护标志 |

派生关系：云 worker 拿 `block_ids + num_computed_tokens` 后**自己派生** slot_mapping/positions——
边不传这些，但原料是边给的。
...
| # | 信息项 | 云自管后的归属 | 说明 |
|---|---|---|---|
| 1 | KVCacheConfig | **每云独立决定** | 不再跨边云统一/clamp；云按自己真实显存定 `num_blocks`；边的 1TiB 虚拟显存 hack 删除（em 边本就无 KV spec） |
| 2 | block_ids（块表） | **云本地生成** | 云 PF 准入时 `allocate_slots` 本地分配，`req_to_blocks` 本地维护；**SO 不再携带任何块 id**。前提正是全量发送——云需全量 token ids 重建链式 hash 才能命中与分配 |
| 3 | num_computed_tokens | **云记账，边持副本** | 云 `ReqRegistry` 是权威；随 c2e/段完成回报边（边用于 chunk 调度与进度展示）；两侧同值靠段协议推进 |
| 4 | prefix 命中判定 | **云本地** | 云查自己的 `cached_block_hash_to_block`；中央 GPI 仅路由参考；边不再做任何命中判断 |
| 5 | finished_req_ids（释放） | **显式 FIN + TTL GC** | 不再搭 SO 便车（多边下某边长期无 SO 到某云是常态）；FIN 独立通道、重传幂等；TTL 兜底边死亡/FIN 丢失 |
| 6 | resumed/抢占恢复 | **控制消息对** | `PreemptNotice`（云→边，L3）、`Pause/Resume`（L2）、`EdgeFreeNotice`（边→云，head_tail 用）；卸载恢复按 `OffloadStore.block_map` H2D 回新块并重建块表 |
| 7 | new_block_ids_to_zero 等内部标志 | **云本地派生** | 块池内部维护细节，本就不需要跨节点 |
| 8 | slot_mapping/positions | **派生逻辑不变，数据源变了** | 云 worker 从"本地 `req_to_blocks` + 本地 num_computed"派生，与今天从 SO 派生是同一段代码路径 |
| 9 | decode/draft 增长槽位 | **云本地随段分配** | DF/DRF 段到达时云 `allocate_slots(增长)`（含 MTP spec token 槽位）；失败走四级处置 |

**云自管后，边→云的段里还剩什么**（KV 归属类信息清零）：
全量 token ids（输入 + 云算哈希的原料）、`lease_id`（唯一的 KV 相关"引用"，仅是凭证号）、
隐藏通道号、优先级、采样参数、draft token ids（MTP 执行用）。

**一句话**：边从"KV 的分配者+账房"（算块、发块表、通知释放）退化为"输入提供者+路由持有者"；
块的生成/命中/增长/释放/驱逐/卸载全部收口在云本地，边云之间线上传递的 KV 相关信息
**只剩一个 lease_id**。

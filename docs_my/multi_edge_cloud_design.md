...
> 版本：v1.2（2026-08-13；v1.0 = 重构为主线 head_tail + 问题总结表；v1.1 = 有效命中 min 规则修正 §4.5；v1.2 = em 内容剥离至独立分册 `multi_edge_cloud_design_embedding_only.md`）
...
> **注 1（req_id 全局唯一）**：vllm 默认 `random_uuid()`，但 OpenAI 协议允许客户端显式指定
> request_id（`chat_completion/protocol.py:352`、`completion/protocol.py:129-132`），两边用户传相同
> id 会撞。边入口统一加 `e{edge_id}-` 前缀（一处改动）使 req_id 成为协议保证的全局唯一键。
> 效果：消息线上不传 edge_id——云从 req_id 前缀解析，收包 channel 也标识来源。
> head_token 同理（边生成时拼 `f"{edge_id}:{uuid}"`）。
...
4. **键天然全局唯一**：req_id/head_token 前缀化（§2.2 注 1），云用裸 id 作键。
...
| Q5 | 需要传 edge_id 吗？ | 不需要：req_id/head_token 前缀化（客户端可指定 req_id 故必须前缀化）+ 收包 channel 标识 | §2.2 注 1 |

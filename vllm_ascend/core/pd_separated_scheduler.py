...
class PDSeparatedScheduler(Scheduler):
    ...
    def _pick_by_state(
        ...
        if state == PrefillState.IDLE:
            ...
            # PDraft尾 is hard-gated on the prefill_draft_down watermark
            # (picked only when its payload has actually arrived), so
            # picking it early never stalls the worker; it clears
            # _force_prefill_draft_last and frees the lane's
            # remote-pending slot, unblocking the next PDraft首.
            if has_pdrl:
        ...
        if state == PrefillState.LOW:
            ...
            # also rank below it.
            # PDraft尾 first: it is data-ready gated (never stalls the
            # worker) and unblocks the next PDraft首 (see IDLE).
            if has_pdrl:
                return self._pick_draft_last_batch(prefill_phase=True)
            if has_ddrl:
                return self._pick_draft_last_batch(prefill_phase=False)
        ...
        if self._has_actionable_prefill_tail():
            return self._pick_prefill_last_batch(ready_only=ready_only)
        # PDraft尾 right behind a ready P尾: it is data-ready gated
        # (never stalls the worker) and unblocks the next PDraft首
        # (see IDLE).
        if has_pdrl:
            return self._pick_draft_last_batch(prefill_phase=True)
        if has_ddrl:

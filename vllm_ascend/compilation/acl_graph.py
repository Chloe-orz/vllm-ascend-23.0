        ...
        with graph_params_scope(self.graph_params, self.draft_graph_params):
            ...
            if entry.aclgraph is None:
                ...
                with ExitStack() as stack:
                    ...
                    logger.info(
                        "[ACLGraphWrapper capture] _EXTRA_CTX.capturing=%s, "
                        "forward_context.capturing=%s, graph_params_id=%s",
                    ...
                    try:
                        ...
                        if gp is not None:
                            logger.info(
                                "[ACLGraphWrapper capture done] graph_params_id=%s, "
                                "attn_params keys=%s, attn_params lens=%s",
                                id(gp),
                                list(gp.attn_params.keys()),
                                {k: len(v) for k, v in gp.attn_params.items()},
                            )

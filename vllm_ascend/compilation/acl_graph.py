...
def update_full_graph_params(
    ...
    # Standard (non edge-cloud) path: keep the baseline behavior unchanged.
    if (
        layer_indices is None
        and graph_params is None
        and draft_graph_params is None
        and unfiltered_attn_metadata is None
    ):
    ...
    with graph_params_scope_no_sync(graph_params, draft_graph_params), set_current_vllm_config(vllm_config):
        ...
        try:
            ...
            # NOTE: update_conv1d_graph_params is only available once the GDN
            # edge-cloud changes are ported; skip gracefully otherwise.
            try:
                from vllm_ascend.ops.gdn import update_conv1d_graph_params
            except ImportError:
                update_conv1d_graph_params = None
            if update_conv1d_graph_params is not None:

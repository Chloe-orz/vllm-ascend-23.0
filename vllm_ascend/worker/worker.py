...
from vllm.v1.core.sched.output import BatchType, GrammarOutput, HiddenChannelType, SchedulerOutput
...
class NPUWorker(WorkerBase):
    ...
    def _execute_model_legacy(
        ...
        intermediate_tensors = None
        forward_pass = scheduler_output.total_num_scheduled_tokens > 0
        if forward_pass and is_first_slice and not get_pp_group().is_first_rank:
        ...
        # NOTE: the legacy path deliberately does not forward
        # ``layer_slice_info`` to the model runner; slicing is only used
        # by the edge-cloud dispatch paths above. This keeps the
        # non-edge-cloud call signature identical to upstream.

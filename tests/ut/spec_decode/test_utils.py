...
def test_explicit_so_baseline_ignores_mutated_device_state():
    """An explicit SO baseline must replace a stale shared-buffer value."""
    num_computed = torch.tensor([1341], dtype=torch.int32)
    ...
    update_num_computed_tokens_for_batch_change(
        ...
        torch.tensor([1344], dtype=torch.int32),
        previous_num_computed_tokens=torch.tensor(
            [1342], dtype=torch.int32
        ),
    )

    assert num_computed.tolist() == [1344]
    assert num_accepted.tolist() == [2]


def test_explicit_so_baseline_corrects_settled_cpu_seq_lens():
    """A settled current SO must not receive the rejection rewind twice."""
    seq_lens = np.array([1346], dtype=np.int32)
    ...
    correct_optimistic_seq_lens_cpu(
        ...
        1,
        previous_num_computed_tokens_np=np.array([1342], dtype=np.int32),
        current_num_computed_tokens_np=np.array([1344], dtype=np.int32),
    )

    np.testing.assert_array_equal(seq_lens, np.array([1346], dtype=np.int32))

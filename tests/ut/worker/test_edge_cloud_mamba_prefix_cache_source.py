...
def _method(name: str) -> ast.FunctionDef:
    module = ast.parse(MODEL_RUNNER.read_text())
    for node in ast.walk(module):
        if isinstance(node, ast.FunctionDef) and node.name == name:
            return node
    raise AssertionError(f"method {name} not found in {MODEL_RUNNER}")
...
def test_cloud_fast_path_restores_deferred_mamba_copy_buffer() -> None:
    ...
    assert (
        ...
        in early_prepare_src
    )
    assert (
        "mamba_preprocess_bufs = cache['mamba_preprocess_bufs']"
        in cloud_fast_path_src
    )


def test_mamba_copy_runs_only_for_metadata_pending_in_this_forward() -> None:
    execute_model_src = _src("execute_model")
    pending_copy_src = ast.unparse(
        _if_branch("execute_model", "mamba_preprocess_bufs is not None")
    )
    edge_fast_path_body = "\n".join(
        ast.unparse(node)
        for node in _if_branch("execute_model", "_fast_path").body
    )

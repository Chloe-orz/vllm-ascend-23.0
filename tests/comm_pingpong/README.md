...
四套脚本，按需取用——一共两个维度（vllm wrapper vs raw、双向 ping-pong vs 单程 one-way）的笛卡尔积：

| | **vllm wrapper** | **raw torch.distributed** |
|---|---|---|
| **双向 RTT** | `pingpong_send.py` / `pingpong_recv.py` | `raw_pingpong_send.py` / `raw_pingpong_recv.py` |
| **单程 send** | `oneway_send.py`   / `oneway_recv.py` | `raw_oneway_send.py`   / `raw_oneway_recv.py` |

- **vllm wrapper** 走的是 `GroupCoordinator.isend_tensor_dict / irecv_tensor_dict`，每次发送前会经过同步的 `send_object(metadata)` 握手，跟生产代码行为一致。
- **raw** 直接调用 `torch.distributed.isend / irecv` 发送一个完整 tensor，不经任何 wrapper，用来定位串行瓶颈到底在 ProcessGroupHCCL 还是 vllm 的握手层。
- **ping-pong** 测端到端 RTT，**one-way** 用 receiver 端的一次 `dist.barrier()` 作 ACK，**用来区分"同方向串行 / 双向流水"**。
...
### raw one-way 版 (原生 isend, 单程 send)

**Receiver (2.2.2.2):**

```bash
python <vllm-ascend>/tests/comm_pingpong/raw_oneway_recv.py \
    --master-addr 1.1.1.1 --master-port 3004 \
    --size-bytes 4194304 --count 1 --iters 20 --warmup 5
```
...
```bash
python <vllm-ascend>/tests/comm_pingpong/raw_oneway_send.py \
    --master-addr 1.1.1.1 --master-port 3004 \
    --size-bytes 4194304 --count 1 --iters 20 --warmup 5
```

#!/usr/bin/env bash
# =============================================================================
# prefill-only 云侧复用（2E1C）启动脚本 —— 边 1（EDGE_ID=1，1 卡，API :8501）
#
# rendezvous 地址/端口取自 registry yaml 的 world 段（优先级 yaml > CLI），
# 本脚本不再传 --master-addr/--master-port。
# 用法：填好下方参数区后直接 bash start_edge1.sh（建议最后启动，
...
API_PORT=8501                 # 边1 对外 OpenAI API 端口
...
vllm serve "$MODEL" \
  ...
  2>&1 | tee /tmp/lwd_2e1c_edge1.log

#!/usr/bin/env bash
# =============================================================================
# prefill-only 云侧复用（2E1C）启动脚本 —— 边 0（EDGE_ID=0，1 卡，API :8500）
#
# rendezvous 地址/端口取自 registry yaml 的 world 段（优先级 yaml > CLI），
# 本脚本不再传 --master-addr/--master-port。
# 用法：填好下方参数区后直接 bash start_edge0.sh（建议先于边1/云启动，
...
# ------------------------------ 参数区（按需调整） --------------------------
PHYSICAL_NPU=0                # 本机物理卡号（ASCEND_RT_VISIBLE_DEVICES）
API_PORT=8500                 # 边0 对外 OpenAI API 端口
...
vllm serve "$MODEL" \
  ...
  --additional-config '{"lwd_config":{"enabled":true,"role":"edge","mode":"prefill_only","edge_head_tail_layers":[0,0]}}' \
  2>&1 | tee /tmp/lwd_2e1c_edge0.log
